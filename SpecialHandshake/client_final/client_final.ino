...
const int NODE_DETECT_THRESHOLD = 2;
...
#define LEFT_BASE_SPEED 210  // 0-255
#define RIGHT_BASE_SPEED 210  // 0-255
...
void handlePathExecution() {
  ...
  char LastCmd = 'F';
  while (true) {
    unsigned long chunkTimeout = millis() + 5000;
    String chunk = "";
    bool received = false;
    while (millis() < chunkTimeout && !received) {
      if (Serial3.available()) {
    ...
    if (!received) {
      Serial.println("Chunk reception timeout!");
      Serial3.println("ERROR: Chunk reception timeout");
      return;
    }
    if (chunk == "END") break;
    ...
    for(int i = 0; i < chunk.length(); i++) {
      ...
      if (cmd == 'L') {
        ...
        if(LastCmd =='F'){// Initial burst to move off the current node
          setMotors(LEFT_BASE_SPEED * 0.8, RIGHT_BASE_SPEED * 0.8);
          delay(300);
        }else{
...
void goForwardThenFollowToNode() {
  ...
  while (tmp < 100) {
    ...
    if (rfidData.length() > 0) { // avoid sending RFID data during the first few loops (when the robot is just starting to move)
...
void leftTurn() {
  ...
  setMotors(LEFT_BASE_SPEED * 0.3, RIGHT_BASE_SPEED * 0.3);
  delay(130);
  ...
  while(state != 2 && tmp < 48) { // while center sensor is not active
    tmp++;
    readSensorsBinary(sense);
    if(state == 0 && sense[0]) { // left sensor detects line first
      state = 1;
      setMotors(0, RIGHT_BASE_SPEED * 0.35); // slow down for better accuracy
    } else if(state == 1 && sense[1]) { // right sensor detects line first
      state = 2;
    }
    delay(5); // small delay for sensor reading stability
    Serial3.print(state);
  }
  stopMotorsPID();
}
...
void rightTurn() {
  ...
  while(state != 2 && tmp < 48) { // while center sensor is not active
    ...
    if(state == 0 && sense[4]) { // left sensor detects line first
      state = 1;
      setMotors(LEFT_BASE_SPEED * 0.35, 0); // slow down for better accuracy
    } else if(state == 1 && sense[3]) { // right sensor detects line first
      state = 2;
    }
    delay(5);
    Serial3.print(state); // small delay for sensor reading stability
  }
  stopMotorsPID();
...
void uTurn() {
  ...
  setMotors(LEFT_BASE_SPEED * 0.65, -RIGHT_BASE_SPEED* 0.65);
  ...
  while(state != 2 && tmp < 75) { // while center sensor is not active
    tmp++;
    readSensorsBinary(sense);
    if(state == 0 && sense[4]) { // left sensor detects line first
      state = 1;
      setMotors(LEFT_BASE_SPEED * 0.2, -RIGHT_BASE_SPEED * 0.2); // slow down for better accuracy
    } else if(state == 1 && sense[3]) { // right sensor detects line first
      state = 2;
    }
    delay(5); // small delay for sensor reading stability
